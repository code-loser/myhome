...
class ExpertCache:
    def __init__(self, make_module: tp.Callable, main_size: int, offload_size: int, buffer_size: int):
        self.module_type = None
        self.module_size = None
        self.device = None
        self.active = False

        self.registered_experts: tp.Dict[tp.Any, ExpertInfo] = dict()

        # 1. 这里不再硬编码 Mixtral，只要是符合接口的 Module 即可
        self.main_modules = [self._check_module(make_module()) for _ in range(main_size)]
        self.main_infos: tp.List[tp.Optional[ExpertInfo]] = [None for _ in range(main_size)]

        # 2. 这里的 UntypedStorage 会根据非量化的参数大小自动调整
        self.offloaded_storages = [
            torch.UntypedStorage(self.module_size).pin_memory(self.device) for _ in range(offload_size)]
        self.offloaded_infos: tp.List[tp.Optional[ExpertInfo]] = [None for _ in range(offload_size)]
        ...
        self.group_infos: tp.Dict[int, EvictionGroupInfo] = defaultdict(EvictionGroupInfo)

    def _check_module(self, module: nn.Module):
        # 只要 Wrapper 实现了 storage 接口即可，不再检查 MixtralExpertWrapper 类型
        if not hasattr(module, 'storage'):
            raise AttributeError("Expert wrapper must have a '.storage' attribute representing its flat weight")
    ...
    def add_expert(self, uid: ExpertUID, module: QwenExpertWrapper, eviction_group: int = 0,
    ...
    def load_experts(
            self, *uids: ExpertUID, unordered: bool = False) -> Iterator[Tuple[ExpertUID, QwenExpertWrapper]]:
        """
        :example:
